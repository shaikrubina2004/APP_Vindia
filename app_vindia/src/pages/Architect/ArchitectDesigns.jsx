...
// ─── Constants ────────────────────────────────────────────────────────────────
const PROJECTS = ["Tower A", "Tower B", "Mall Complex", "Residential Phase 1", "Metro Depot"];
...
const emptyNode = () => ({
  state: "pending",
  sentAt: null, returnedAt: null, approvedAt: null,
  sentBy: "", note: "", revision: "", returnedFileUrl: "",
});
...
const makeDrawing = ({ id, project, name, drawingType, revision,
  stage = "Draft", status = "Pending", description = "", fileName = "", previewUrl = "" }) => ({
  id, project, name, drawingType, revision, stage, status, description,
  fileName, previewUrl, createdAt: now(), updated: now(), approvedRevision: "",
  workflow: emptyWorkflow(),
  history: [{ rev: revision, stage, status, updated: now(), previewUrl, note: description || "Initial upload" }],
  actionLog: [{ ts: now(), action: "Created", stage: "—", note: description || "Drawing created" }],
});

const createSeedProject = () => ({
  id: "PRJ-001", name: "Tower A", status: "Pending",
  drawings: [makeDrawing({
    id: "DWG-001", project: "Tower A", name: "Tower A Working Drawing",
    drawingType: "Working Drawing", revision: "R1", description: "Initial working drawing.",
  })],
});
...
const stageActions = (d, step) => {
  ...
  const siteSentOrMore = ["sent", "returned", "approved"].includes(w.site.state);
...
if (isDetailed) {
  ...
  switch (step) {
    ...
    case "mep":
      return {
        canSend:         structDone && (node.state === "pending" || node.state === "rejected"),
        canReturn:       node.state === "sent",
        canApprove:      node.state === "sent" || node.state === "returned",
        canReject:       node.state === "sent" || node.state === "returned",
    ...
    case "qs":
      return {
        ...
        canReturn:  false, canApprove: false, canReject: false,
        canView,
      };
    ...
    case "pm":
  return {
    canSend:    qsSentOrMore && siteSentOrMore &&
                (node.state === "pending" || node.state === "rejected" || node.state === "returned"),
    canReturn:  node.state === "sent",
    ...
    case "client": {
      ...
      const pmActed = ["returned", "rejected"].includes(w.pm.state) === false
        ? w.pm.state === "sent" || w.pm.state === "returned"
        : false;
      const pmSentOrReturned = ["sent", "returned"].includes(w.pm.state);
      return {
        canSend:    pmSentOrReturned && (node.state === "pending" || node.state === "rejected"),
        canReturn:  false,
        canApprove: node.state === "sent",
        canReject:  node.state === "sent",
        canView,
    ...
    default:
      return { canSend: false, canReturn: false, canApprove: false, canReject: false, canView: false };
  }
};
...
const initialState = {
  ...
  selectedProjectId: "PRJ-001", selectedDrawingId: "DWG-001",
  search: "", projectFilter: "All", statusFilter: "All",
  modal: null, toast: null,
  uploadForm: {
    file: null, project: "Tower A", projectStatus: "Pending",
    drawingName: "", drawingType: "Working Drawing", revision: "R1", previewUrl: "",
  },
  revisionFile: null,
};
...
function reducer(state, action) {
  switch (action.type) {
    ...
    case "BACK_TO_LIST":  return { ...state, modal: "project" };
    case "SET_TOAST":     return { ...state, toast: { id: uid(), ...action.toast } };
    case "CLEAR_TOAST":   return { ...state, toast: null };
    case "SET_UPLOAD_FORM": return { ...state, uploadForm: { ...state.uploadForm, [action.field]: action.value } };
    ...
    case "SET_REVISION_FILE": return { ...state, revisionFile: action.file };

    case "ADD_DRAWING": {
      const exists = state.projects.some((p) => p.name === action.projectName);
      const projects = exists
        ? state.projects.map((p) => p.name !== action.projectName ? p
            : { ...p, drawings: [action.drawing, ...p.drawings] })
        : [...state.projects, {
            id: `PRJ-${String(state.projects.length + 1).padStart(3, "0")}`,
            name: action.projectName, status: action.projectStatus || "Pending",
      ...
      return {
        ...
        selectedDrawingId: action.drawing.id, modal: "project",
      };
    }

    case "UPDATE_DRAWING": {
      const projects = state.projects.map((p) =>
        p.id !== action.projectId ? p
          : { ...p, drawings: p.drawings.map((d) => d.id === action.drawing.id ? action.drawing : d) });
      const updatedProjects = projects.map((p) => {
        if (p.id !== action.projectId) return p;
        const allApproved = p.drawings.length > 0 && p.drawings.every((d) => d.status === "Approved");
        return { ...p, status: allApproved ? "Approved" : p.status };
      });
      return {
        ...state, projects: updatedProjects,
        selectedProjectId: action.projectId, selectedDrawingId: action.drawing.id,
      };
    }

    case "DELETE_DRAWING": {
      const projects = state.projects
        .map((p) => p.id !== action.projectId ? p
          : { ...p, drawings: p.drawings.filter((d) => d.id !== action.drawingId) })
        .filter((p) => p.drawings.length > 0);
      const next = projects.find((p) => p.id === action.projectId) || projects[0] || null;
      return {
        ...state, projects, selectedProjectId: next?.id || "",
        selectedDrawingId: next?.drawings[0]?.id || "", modal: next ? state.modal : null,
      };
    }

    case "SELECT_PROJECT": return {
      ...state, selectedProjectId: action.projectId,
      selectedDrawingId: action.drawingId || state.selectedDrawingId, modal: "project",
    };
    case "SELECT_DRAWING": return {
      ...state, selectedProjectId: action.projectId,
      selectedDrawingId: action.drawingId, modal: "drawing",
    };
  ...
  return (
    <div className="revision-list">
      {drawing.history.map((h) => (
        <button key={`${h.rev}-${h.updated}`} type="button" className="revision-item"
...
function phaseBadge(step) {
  if (["structural", "mep"].includes(step))   return { label: "Phase 1", cls: "phase-1" };
  if (["qs", "site"].includes(step))           return { label: "Phase 2", cls: "phase-2" };
    ...
    <div className={`wf-stage wf-stage--${node.state}`}>
      ...
      {node.sentAt && (
        <div className="wf-stage-meta">
          ...
          {node.revision   && <div className="wf-stage-rev">Rev: {node.revision}</div>}
        </div>
      )}
      ...
      <div className="wf-stage-actions">
        ...
        {node.returnedFileUrl && (
          <a href={node.returnedFileUrl} target="_blank" rel="noreferrer"
        ...
        {step === "mep" && drawing.workflow.structural.returnedFileUrl && (
          <a href={drawing.workflow.structural.returnedFileUrl} target="_blank" rel="noreferrer"
        ...
        {actions.canSend && (
          <button className="btn btn--info btn--sm" onClick={() => onAction(step, "send")}>
            ↑ Send
          </button>
        )}
        ...
        {actions.canReturn && isFileStage && (
          <>
            <input ref={fileRef} type="file" accept="*/*" className="hidden-file"
              onChange={(e) => {
...
function AddDrawingModal({ project, onClose, onAdd, toastFn }) {
  ...
  const handleAdd = () => {
    ...
    const drawing = makeDrawing({
      id: `DWG-${Date.now()}`, project: project.name,
      name: name.trim(), drawingType: type, revision: revision.trim(),
      stage: "Draft", status: "Pending", fileName: file.name, previewUrl,
    });
          ...
          <div className="form-grid">
            <div className="form-field full">
              ...
              <input className="input" value={name}
                onChange={(e) => setName(e.target.value)} placeholder="e.g. Structural Plan" />
            </div>
            <div className="form-field full">
            ...
            <div className="form-field full">
              ...
              <input className="input" value={revision}
                onChange={(e) => setRevision(e.target.value)} placeholder="e.g. R1" />
            </div>
            <div className="form-field full">
      ...
      <div className="modal modal-medium" onMouseDown={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
        ...
        <div className="modal-body">
          <div className="info-grid" style={{ marginTop: 8 }}>
            <span className="info-label">Status</span>  <span className={badgeClass(revision.status)}>{revision.status}</span>
            <span className="info-label">Stage</span>   <span>{revision.stage}</span>
            <span className="info-label">Updated</span> <span>{fmt(revision.updated)}</span>
            <span className="info-label">Note</span>    <span>{revision.note || "—"}</span>
          </div>
          {revision.previewUrl && (
...
export default function ArchitecturalDrawingManagementSystem() {
  ...
  const selectedDrawing = useMemo(
    () => selectedProject?.drawings.find((d) => d.id === state.selectedDrawingId)
      || selectedProject?.drawings[0] || null,
    [selectedProject, state.selectedDrawingId]
  );
  useEffect(() => { setSelectedRevision(null); }, [selectedDrawing?.id]);
  ...
  const closeModal  = () => { dispatch({ type: "CLOSE_MODAL" }); setShowAddModal(false); };
 ...
 const handleWorkflowAction = (step, actionType, returnedFile = null) => {
  ...
  break;
      ...
      case "return":
        ...
        node.returnedFileUrl = returnedFile ? URL.createObjectURL(returnedFile) : "";
  ...
  break;
    ...
    next.updated = now();
    next.actionLog = [
      { ts: now(), action: actionType.toUpperCase(), stage: step, note: `${STAGE_LABELS[step] || step} — ${actionType}` },
      ...next.actionLog,
    ];
  ...
  const handleUploadRevision = () => {
    ...
    createdUrlsRef.current.push(previewUrl);
    next.revision = newRev; next.previewUrl = previewUrl;
    next.fileName = state.revisionFile.name; next.updated = now();
    next.stage    = `Revised — ${newRev}`; next.status = "Revision";
    next.history  = [{
      rev: newRev, stage: next.stage, status: "Revision",
      updated: now(), previewUrl, note: revNote || "New revision.",
    }, ...next.history];
    if (revTargetStage && next.workflow[revTargetStage]) next.workflow[revTargetStage] = emptyNode();
    next.actionLog = [{
      ts: now(), action: "New Revision Uploaded",
      stage: revTargetStage || "—", revision: newRev,
      note: revNote || "Architect uploaded revised drawing.",
    }, ...next.actionLog];
    dispatch({ type: "UPDATE_DRAWING", projectId: selectedProject.id, drawing: next });
    dispatch({ type: "SET_REVISION_FILE", file: null });
    setRevNote(""); setRevTargetStage("");
  ...
  const handleGlobalUpload = () => {
    ...
    const drawing = makeDrawing({
      ...
      project, name: drawingName.trim(), drawingType, revision: revision.trim(),
      stage: "Draft", status: "Pending", fileName: file.name, previewUrl,
    });
  ...
  return (
    <div className="page">
      ...
      <input ref={globalUploadRef} type="file" accept="*/*" className="hidden-file"
        onChange={(e) => onGlobalFileChange(e.target.files?.[0] || null)} />
      <input ref={revisionUploadRef} type="file" accept="*/*" className="hidden-file"
      ...
      <header className="header">
        <div className="header-text">
          ...
          <p className="header-sub">Structural · MEP · QS · Site · PM · Client</p>
        </div>
      ...
      <div className="filters">
        <input className="input filters-search" placeholder="Search project…"
          value={state.search}
          onChange={(e) => dispatch({ type: "SET", field: "search", value: e.target.value })} />
        <select className="input filters-select" value={state.projectFilter}
          onChange={(e) => dispatch({ type: "SET", field: "projectFilter", value: e.target.value })}>
          <option value="All">All Projects</option>
          {PROJECTS.map((p) => <option key={p} value={p}>{p}</option>)}
        </select>
        <select className="input filters-select" value={state.statusFilter}
          onChange={(e) => dispatch({ type: "SET", field: "statusFilter", value: e.target.value })}>
      ...
      <div className="table-card">
        <table className="table">
          <thead>
            <tr>
              <th>Project ID</th><th>Project Name</th><th>Status</th>
              <th>Drawings</th><th>Approved</th><th>Pending</th><th>Revision</th>
            </tr>
          </thead>
          <tbody>
            {filteredProjects.length === 0
              ...
              : filteredProjects.map((project) => {
                  ...
                  const revision = project.drawings.filter((d) => d.status === "Revision").length;
                  return (
                    <tr key={project.id}
                      className={`${state.selectedProjectId === project.id ? "row--selected" : ""} ${project.status === "Approved" ? "row--approved" : ""}`}
                      onClick={() => openProject(project.id)} style={{ cursor: "pointer" }}>
                      ...
                      <td>{approved}</td><td>{pending}</td><td>{revision}</td>
                    </tr>
                  );
                })
            }
      ...
      {state.modal === "project" && selectedProject && (
        <Modal title={`Project: ${selectedProject.name}`} onClose={closeModal} small>
          <div className="project-meta-bar">
            ...
            <span>Status: <span className={badgeClass(selectedProject.status)}>{selectedProject.status}</span></span>
                ...
                {selectedProject.drawings.map((drawing) => (
                  <div key={drawing.id}
                    ...
                    <div className="drawing-card-footer">
                      <button className="btn btn--ghost btn--sm"
                        onClick={(e) => { e.stopPropagation(); openDrawing(selectedProject.id, drawing.id); }}>
                        View
                      </button>
                      <button className="btn btn--danger btn--sm"
                        onClick={(e) => { e.stopPropagation(); removeDrawing(selectedProject.id, drawing.id); }}>
                        Remove
                      </button>
                    </div>
                  </div>
      ...
      {showAddModal && selectedProject && (
        <AddDrawingModal project={selectedProject} onClose={() => setShowAddModal(false)}
          onAdd={handleAddDrawing} toastFn={toast} />
      )}
            ...
            <div className="detail-left">
              <div className="card">
                ...
                <div className="info-grid">
                  <span className="info-label">Name</span>        <span>{selectedDrawing.name}</span>
                  <span className="info-label">Project</span>     <span>{selectedDrawing.project}</span>
                  <span className="info-label">Type</span>        <span>{selectedDrawing.drawingType}</span>
                  <span className="info-label">Current Rev</span> <span><strong>{selectedDrawing.revision}</strong></span>
                  <span className="info-label">Status</span>      {selectedDrawing.drawingType !== "Detailed Drawing" && (
  <span className={badgeClass(selectedDrawing.status)}>
    {selectedDrawing.status}
  </span>
)}
                  <span className="info-label">Stage</span>       <span>{displayStage(selectedDrawing)}</span>
              ...
              <div className="card">
                ...
                <div className="upload-file-row">
                  <button className="btn btn--ghost btn--sm"
                    onClick={() => revisionUploadRef.current?.click()}>Choose File</button>
                  {state.revisionFile && <span className="muted file-name">{state.revisionFile.name}</span>}
                </div>
                <select className="input" value={revTargetStage}
                  onChange={(e) => setRevTargetStage(e.target.value)} style={{ marginTop: 8 }}>
                ...
                <input className="input" placeholder="Note (optional)"
                  value={revNote} onChange={(e) => setRevNote(e.target.value)} style={{ marginTop: 8 }} />
                <div style={{ marginTop: 10 }}>
                  <button className="btn btn--primary btn--sm" onClick={handleUploadRevision}>
                    Upload Revision
                  </button>
                </div>
              </div>
              ...
              <div className="card">
                ...
                <RevisionHistory drawing={selectedDrawing} onSelectRevision={setSelectedRevision} />
              </div>
            </div>
              ...
              <div className="card card--stretch">
                <div className="card-title">
                  ...
                  <span style={{ marginLeft: 10, fontSize: 11, fontWeight: 400, color: "var(--muted)" }}>
                    {selectedDrawing.drawingType === "Working Drawing" ? "6 stages" : "4 stages"}
                  </span>
                </div>
                
                <div className="wf-stages">
                  {stageOrderFor(selectedDrawing).map((step) => (
                    <WorkflowStageCard key={step} drawing={selectedDrawing} step={step}
          ...
          {selectedRevision && (
            <RevisionPopup revision={selectedRevision} onClose={() => setSelectedRevision(null)} />
          )}
        </Modal>
      )}
        ...
        <Modal title="Upload Drawing" onClose={closeModal}
          ...
          <div className="form-grid">
            <div className="form-field full">
              ...
              <div className="upload-file-row">
                <button className="btn btn--ghost btn--sm"
                  onClick={() => globalUploadRef.current?.click()}>Choose File</button>
                {state.uploadForm.file && <span className="muted file-name">{state.uploadForm.file.name}</span>}
              </div>
            </div>
            <div className="form-field">
              <label className="form-label">Project</label>
              <select className="input" value={state.uploadForm.project}
                onChange={(e) => dispatch({ type: "SET_UPLOAD_FORM", field: "project", value: e.target.value })}>
                {PROJECTS.map((p) => <option key={p} value={p}>{p}</option>)}
              </select>
            </div>
            <div className="form-field">
              <label className="form-label">Project Status</label>
              <select className="input" value={state.uploadForm.projectStatus}
                onChange={(e) => dispatch({ type: "SET_UPLOAD_FORM", field: "projectStatus", value: e.target.value })}>
                {STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
            <div className="form-field">
              <label className="form-label">Drawing Name</label>
              <input className="input" value={state.uploadForm.drawingName}
                placeholder="e.g. Tower A Floor Plan"
                onChange={(e) => dispatch({ type: "SET_UPLOAD_FORM", field: "drawingName", value: e.target.value })} />
            </div>
            <div className="form-field">
              <label className="form-label">Drawing Type</label>
              <select className="input" value={state.uploadForm.drawingType}
                onChange={(e) => dispatch({ type: "SET_UPLOAD_FORM", field: "drawingType", value: e.target.value })}>
                {DRAWING_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
              </select>
            </div>
            <div className="form-field">
              <label className="form-label">Revision</label>
              <input className="input" value={state.uploadForm.revision} placeholder="e.g. R1"
                onChange={(e) => dispatch({ type: "SET_UPLOAD_FORM", field: "revision", value: e.target.value })} />
            </div>
          </div>
        </Modal>
      )}
          ...
          <div className="table-card">
            <table className="table">
              <thead>
                <tr>
                  <th>Project ID</th><th>Project Name</th><th>Status</th>
                  <th>Drawings</th><th>Approved</th>
                </tr>
              </thead>
              <tbody>
                {state.projects.filter((p) => p.status === "Approved").length === 0
