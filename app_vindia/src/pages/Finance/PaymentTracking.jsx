...
export default function PaymentTracking() {
  ...
  /*
   * Normalize role so the Accountant check works with values such as:
   * accountant
   * Accountant
   * ACCOUNTANT
   * finance-manager
  ...
  /*
   * Load payment-related data.
   *
   * IMPORTANT:
   * Accountant does NOT have access to Finance Settings.
   * Therefore getSettings() is only called for non-Accountants.
   */
  const loadAll = useCallback(async () => {
    ...
    try {
      const [payRes, vendorRes, projRes, invRes, settingsRes] =
        await Promise.all([
      ...
      setBankAccounts(
        settingsRes.data.data.bankAccounts || []
      );
    } catch (err) {
      setError(
  ...
  /*
   * Accountant is allowed to create payments because the backend
   * currently permits Accountant payment creation.
   */
  ...
  /*
   * Accountant is allowed to update payment status.
   * Therefore pending -> completed is available to Accountant.
   */
  const handleMarkCompleted = async (payment) => {
    try {
      await financeService.updatePayment(
        payment.id,
        {
          status: "completed",
        }
  ...
  /*
   * Delete remains restricted to Finance Manager / authorised
   * management roles.
   */
  const handleDelete = async (payment) => {
    // Accountant cannot delete payment records.
    // The backend also restricts this operation to management roles.
    if (isAccountant) {
      return;
    }
    ...
    if (
      !window.confirm(
        `Delete this ${fmt(payment.amount)} payment record?`
      )
    ) {
      return;
    }
  ...
  payments.forEach((p) => {
    ...
    if (p.payment_type === "outgoing") {
      if (p.status === "completed") {
        trendMap[key].paid += Number(
          p.amount
        );
      } else if (p.status === "pending") {
        trendMap[key].pending += Number(
          p.amount
        );
      }
    }
    ...
    >
      ...
      <div className="pt-header">
        <div>
          ...
          >
            {isAccountant
              ? "Record and monitor financial payments"
              : "Monitor and manage financial payments"}
          </p>
        </div>

        {/* Accountant is allowed to create payments.
            Finance Manager also retains this button. */}
        <button
      ...
      {isAccountant && (
        ...
        >
          ...
          pending payments and scheduled obligations.
          Bank-account configuration and payment
          deletion remain restricted to authorised
          management roles.
        </div>
      )}
      ...
      <div className="pt-summary">
        <div
          className="pt-summary-card"
          style={{ "--accent": "#059669" }}
        >
          <div className="pt-summary-top">
            <p className="pt-summary-label">
              Total Paid (Outgoing)
            </p>
          </div>

          <p className="pt-summary-value">
            {fmt(totalPaid)}
          </p>
        </div>
      ...
      {trend.length > 0 && (
        <div className="pt-chart-card">
          <div className="pt-chart-head">
            <h3>
              Monthly Payment Trend (Outgoing)
            </h3>
            ...
            {recent.map((p) => (
              ...
              >
                ...
                <div className="pt-payment-right">
                  <p className="pt-payment-amount">
                    {fmt(p.amount)}
                  </p>

                  <span
                    className="pt-payment-status"
                    style={{
                      background:
                        getStatusBgColor(
                          p.status
          ...
          <tbody>
            {filteredHistory.map((p) => (
              ...
              >
                ...
                <td className="pt-vendor">
                  {p.vendor_name ||
                    p.invoice_number ||
                    "—"}
                </td>

                <td className="pt-category">
                  {p.payment_type ===
                  "incoming"
                    ? "Incoming"
                    : "Outgoing"}
        ...
        <div className="pt-schedule-list">
          {scheduled.map((p) => {
            ...
            return (
              <div
                key={p.id}
                className={`pt-schedule-item ${
                  overdue
                    ? "pt-schedule-item--due-soon"
                    : "pt-schedule-item--upcoming"
                }`}
              >
                <div className="pt-schedule-left">
                  <div className="pt-schedule-days">
                    {overdue
                      ? `${Math.abs(
                          days
                        )}d late`
                  ...
                  <div className="pt-schedule-content">
                    <p className="pt-schedule-vendor">
                      {p.vendor_name ||
                        p.invoice_number ||
                        "—"}
                    </p>

                    <p className="pt-schedule-meta">
                      Due:{" "}
                      {formatDate(
                        p.payment_date
                      )}
                ...
                <div className="pt-schedule-right">
                  <p className="pt-schedule-amount">
                    {fmt(p.amount)}
                  </p>

                  <span
                    className="pt-schedule-status"
                    style={{
                      background: overdue
                        ? "#fee2e2"
                        : "#dbeafe",
  ...
  return (
    <div className="pt-methods">
      ...
      {bankAccounts.length === 0 ? (
        ...
        >
          {isAccountant
            ? "Bank account details are managed by the Finance Manager."
        ...
        <div className="pt-methods-grid">
          {bankAccounts.map((b) => (
            <div
              key={b.id}
              className="pt-method-card pt-method-card--active"
            >
              <div className="pt-method-header">
                <p className="pt-method-type">
                  {b.is_primary
                    ? "Primary Account"
                    : "Bank Account"}
                </p>
              ...
              <p className="pt-method-provider">
                {b.bank_name}
              </p>

              <p className="pt-method-account">
                {b.account_holder} • A/C
                ending in{" "}
                {String(
                  b.account_number
                ).slice(-4)}
    ...
    >
      ...
      >
        ...
        <div className="pt-modal-actions">
          ...
          {/* Accountant CAN mark pending payment completed */}
          {payment.status === "pending" && (
            <button
              className="pt-btn-primary"
              onClick={() =>
                onMarkCompleted(payment)
              }
            >
              Mark Completed
            </button>
          ...
          {/* Delete remains management-only */}
          {!isAccountant && (
            <button
              className="pt-btn-outline"
              style={{
