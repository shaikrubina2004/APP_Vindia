...
const APPROVAL_TYPES = [
  { value: "work",       label: "Work Approval",      desc: "Completed work ready for PM review" },
  { value: "inspection", label: "Inspection Request",  desc: "Request QC inspection before proceeding" },
  { value: "material",   label: "Material Approval",   desc: "Material on site — request acceptance" },
  { value: "method",     label: "Method Statement",    desc: "Approval of construction method" },
];
...
function enqueue(p) { const q = ls.load(QUEUE_KEY)||[]; q.push({id:`q_${Date.now()}`,payload:p,createdAt:new Date().toISOString()}); ls.save(QUEUE_KEY,q); }
async function flushQueue() { const q=ls.load(QUEUE_KEY); if(!Array.isArray(q)||!q.length) return; const rem=[]; for(const item of q){try{const r=await api.post("/approvals",item.payload); if(!r||(r.status&&r.status>=400)) throw new Error();}catch{rem.push(item);}} ls.save(QUEUE_KEY,rem); }
function stableKey(it) { return it?.id!=null?String(it.id):`${it?.title||""}|${it?.type||""}|${it?.createdAt||""}`; }
function validate(f) { const e={}; if(!f.title||f.title.trim().length<3) e.title="Title required (min 3 chars)"; if(!f.type) e.type="Select approval type"; return e; }

function StatusBadge({s}) {
  const c=STATUS_CFG[s]||STATUS_CFG.pending;
  return <span className="aw-badge" style={{background:c.bg,color:c.color,border:`0.5px solid ${c.border}`}}>{c.label}</span>;
}
...
export default function ApprovalWorkflow() {
  ...
  useEffect(()=>{ alive.current=true; loadList(); flushQueue().catch(()=>{}); return()=>{ alive.current=false; }; },[]);
  ...
  async function loadList() {
    ...
    try {
      ...
      const seen=new Set();
      setApprovals(raw.filter(it=>{ const k=stableKey(it); if(seen.has(k)) return false; seen.add(k); return true; }));
    } catch {}
    finally { if(alive.current) setLL(false); }
  }

  const setF=useCallback((k,v)=>{ setForm(f=>({...f,[k]:v})); setErrors(e=>{ const c={...e}; delete c[k]; return c; }); setStatus(""); },[]);
  const handleFiles=useCallback(e=>{ setForm(f=>({...f,attachments:[...f.attachments,...Array.from(e.target.files||[])]})); e.target.value=null; },[]);
  ...
  const submit=useCallback(async ev=>{
    ...
    const opt={id:`local_${Date.now()}`,...form,status:"pending",createdAt:new Date().toISOString(),optimistic:true};
    setApprovals(s=>[opt,...s]);
    try {
      let res;
      if(form.attachments.length){
        const fd=new FormData();
        ["type","title","description","zone","activity","linked_task","linked_rfi","linked_diary","assign_to_role"].forEach(k=>fd.append(k,form[k]||""));
      ...
      push(`Approval request submitted: "${form.title}"`,"approval",{linked_ref:form.linked_task||""});
      await loadList();
      setForm({...BLANK});
  ...
  const filtered=useMemo(()=>{
    ...
    if(search.trim()){ const q=search.toLowerCase(); list=list.filter(it=>(it.title||"").toLowerCase().includes(q)||(it.zone||"").toLowerCase().includes(q)); }
  ...
  return (
    <div className="aw-page">
      ...
      <div className="aw-page-header">
        <div>
          <div className="aw-eyebrow">Quality Control</div>
          <h1 className="aw-title">Approval Requests</h1>
          <div className="aw-sub">Site Engineer → Requests → QC / PM → Approve / Reject</div>
        </div>
      ...
      <div className="aw-flow-strip">
        {[
          ...
          null,
          {label:"QC / PM",sub:"Reviews",color:"#085041"},
          null,
          {label:"Approved ✓",sub:"Work proceeds",color:"#085041",bg:"#E1F5EE"},
          {label:"or"},
          {label:"Rejected ✗",sub:"Rework required",color:"#791F1F",bg:"#FCEBEB"},
        ].map((s,i)=>
          !s
      ...
      <div className="aw-layout">
              ...
              <form onSubmit={submit} noValidate>
                ...
                <div className="aw-form-section">
                  ...
                  <div className="aw-type-grid">
                    {APPROVAL_TYPES.map(t=>(
                      <div key={t.value} className={`aw-type-card${form.type===t.value?" aw-type-card--active":""}`} onClick={()=>setF("type",t.value)}>
                ...
                <div className="aw-form-section">
                  ...
                  <div className="aw-field aw-mb">
                    ...
                    <textarea className="aw-textarea" value={form.description} onChange={e=>setF("description",e.target.value)} placeholder="What specifically needs to be inspected or approved? Include location, drawing ref, and relevant specs."/>
                  </div>
                  <div className="aw-grid-2 aw-mb">
                    <div className="aw-field"><label className="aw-label">Zone / Location</label><input className="aw-input" value={form.zone} onChange={e=>setF("zone",e.target.value)} placeholder="e.g. Level 3 / Grid B2"/></div>
                    <div className="aw-field"><label className="aw-label">Activity</label><input className="aw-input" value={form.activity} onChange={e=>setF("activity",e.target.value)} placeholder="e.g. Column rebar fixing"/></div>
                  </div>
                ...
                <div className="aw-form-section">
                  ...
                  <div className="aw-grid-3">
                    {[["linked_task","Linked Task","TASK-001"],["linked_rfi","Linked RFI","RFI-007"],["linked_diary","Linked Diary","YYYY-MM-DD"]].map(([k,label,ph])=>(
                      <div key={k} className="aw-field"><label className="aw-label">{label}</label><input className="aw-input" value={form[k]} onChange={e=>setF(k,e.target.value)} placeholder={ph}/></div>
                    ))}
                  </div>
                </div>
                ...
                <div className="aw-submit-row">
                  <button type="submit" className="aw-btn aw-btn--primary" disabled={submitting}>{submitting?"Submitting…":"Submit for Approval"}</button>
                  {status&&<span className={`aw-status${status.includes("✓")?" aw-status--ok":status.includes("Offline")?" aw-status--err":" aw-status--saving"}`}>{status}</span>}
                </div>
              </form>
            </div>
          </div>
        ...
        <aside className="aw-aside">
          <div className="aw-aside-card">
            ...
            {[["Pending Review",stats.pending,"#BA7517"],["Approved",stats.approved,"#085041"],["Rejected",stats.rejected,"#791F1F"],["Total",approvals.length,"var(--c-navy-700,#0A4174)"]].map(([l,v,c])=>(
              <div key={l} className="aw-aside-row"><span>{l}</span><strong style={{color:c}}>{v}</strong></div>
            ))}
          </div>
          <div className="aw-aside-card">
            ...
            <ul className="aw-rules-list">
              ...
              <li>Concrete pours need inspection first</li>
              <li>Rejected items must be re-submitted with revisions</li>
              <li>Link to Task, RFI, or Diary for traceability</li>
            </ul>
          </div>
        </aside>
      </div>
