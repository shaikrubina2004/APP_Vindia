...
export default function RFIPage() {
  ...
  const visible = rfis.filter((r) => {
    ...
    const matchSearch =
      ...
      ROLE_LABELS[r.raised_by_role]?.toLowerCase().includes(search.toLowerCase()) ||
      ROLE_LABELS[r.assigned_to_role]?.toLowerCase().includes(search.toLowerCase());
    ...
    <div className="rfi-page">
      ...
      <div className="rfi-page-header">
        <div>
          ...
          <p className="rfi-page-sub">Requests for Information across all roles</p>
        </div>
      ...
      <div className="rfi-stats">
        {[
          ...
          <div className="rfi-stat-card" key={s.label}
               style={{ borderLeft: `4px solid ${s.color}` }}>
            <span className="rfi-stat-label">{s.label}</span>
            <span className="rfi-stat-value" style={{ color: s.color }}>{s.value}</span>
          </div>
        ))}
      </div>
      ...
      <div className="rfi-view-tabs">
        {["all", "sent", "received"].map((v) => (
          <button key={v}
            ...
            onClick={() => setView(v)}>
            {v === "all" ? "All RFIs" : v === "sent" ? "Raised by Me" : "Assigned to Me"}
          </button>
        ))}
      </div>
              ...
              {visible.map((r) => {
                ...
                return (
                  <tr key={r.id}
                    ...
                    <td>
                      <span className={`rfi-role-chip ${isMyRFI ? "chip-me" : ""}`}>
                    ...
                    <td>
                      <span className="rfi-priority"
                            style={{ color: PRIORITY_COLOR[r.priority] || "#64748b" }}>
                        ● {r.priority}
                      </span>
                    </td>
