...
export function NotificationProvider({ children }) {
  const [notifications, setNotifications] = useState(() => ls.load(STORAGE_KEY) || []);
  ...
  // Poll server every 60s for new notifications
  useEffect(() => {
    alive.current = true;
    fetchRemote();
    pollRef.current = setInterval(fetchRemote, 60_000);
  ...
  return (
    <NotificationContext.Provider value={{ notifications, push, markRead, markAllRead, dismiss, unreadCount }}>
      {children}
